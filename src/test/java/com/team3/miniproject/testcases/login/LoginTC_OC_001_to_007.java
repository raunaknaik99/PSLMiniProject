...
public class LoginTC_OC_001_to_007 {
  ...
  public void testCase001To005() throws IOException, InterruptedException {
	...
	@Test
	//TC_OC_LR_006 - To verify that Login page is inaccessible when user is logged in
	public void testCase006() throws IOException {
		...
		String password=reader.getPassword();  
		login1.login(emailId,password);
		//Trying to access login page using URL
	    ...
	    //Assert that Logged in user cannot access login page and is redirected to 'My Account' page.
		Assert.assertEquals(actualTitle, expectedTitle);
	...
	public void testCase007() throws IOException {
		...
		//Assert that Logged in user does not get logged out after clicking the back button on the browser.
		Assert.assertEquals(actualTitle, expectedTitle);
	}

	@BeforeMethod
	public void beforeMethod(Method m) {
		//Generating extent report with time stamp
